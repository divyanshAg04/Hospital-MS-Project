...
async function seed() {
  ...
  try {
    ...
    // Sync all
    await hydrateCollection('patients', []);
    await hydrateCollection('doctors', []);
    await hydrateCollection('appointments', []);
    await hydrateCollection('medicalrecords', []);
    await hydrateCollection('medicines', []);
    await hydrateCollection('invoices', []);
    await hydrateCollection('activitylogs', []);
